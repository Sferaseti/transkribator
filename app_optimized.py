...
def extract_audio_from_video(video_path):
    """Extract complete audio track from video file"""
    try:
        ...
        temp_audio_path = os.path.join(tempfile.gettempdir(), f"extracted_audio_{uuid.uuid4().hex}.wav")
        ...
        logger.info(f"Audio extracted successfully. Duration: {duration:.1f}s")
    ...
    try:
        ...
        try:
            ...
            if is_video_file(file.filename):
                ...
                audio_file_path, total_duration = extract_audio_from_video(temp_file_path)
                ...
                if total_duration > chunk_duration:
                    ...
                    audio_chunks = chunk_audio_file(audio_file_path, chunk_duration)
                    ...
                    if language and language != "auto":
                        result = model.transcribe(audio_file_path, language=language, task=task)
                    else:
                ...
                try:
                    # Get audio duration using ffmpeg
                    result = subprocess.run([
                        'ffmpeg', '-i', audio_file_path, '-f', 'null', '-'
            ...
            return jsonify({
                ...
                'optimization': 'audio_extraction' if is_video_file(file.filename) else 'direct_processing'
            })
            ...
            if temp_file_path and os.path.exists(temp_file_path):
                try:
                    ...
                    logger.info("Cleaned up original uploaded file")
                except Exception as e:
                    logger.warning(f"Could not remove temporary file {temp_file_path}: {str(e)}")
            
            # Clean up extracted audio file (if different from temp_file_path)
            if audio_file_path and audio_file_path != temp_file_path and os.path.exists(audio_file_path):
                try:
                    os.remove(audio_file_path)
                    logger.info("Cleaned up extracted audio file")
                except Exception as e:
                    logger.warning(f"Could not remove audio file {audio_file_path}: {str(e)}")
